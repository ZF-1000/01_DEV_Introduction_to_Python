...
def start(update, _):
    ...
    update.message.reply_text('Добро пожаловать в калькулятор.', reply_markup=markup_key)
    return CHOICE
...
if __name__ == '__main__':
    updater = Updater(TOKEN)  # создаем Updater и передаем ему токен вашего бота
    dispatcher = updater.dispatcher  # получаем диспетчера для регистрации обработчиков

    # Определяем обработчик разговоров ConversationHandler
    # с состояниями CHOICE, RATIONAL_ONE, RATIONAL, RATIONAL_TWO, OPERATIONS_RATIONAL, OPERATIONS_COMPLEX, COMPLEX_ONE, COMPLEX_TWO
